...
func postAlertsJSON(context *gin.Context) {
	...
	if _, err := stmt.Exec(newAlert.IdAlert, newAlert.Username, newAlert.Price, newAlert.Condition, newAlert.StartDate, newAlert.EndDate, newAlert.IsActive); err != nil {
		log.Fatal(err)
	}
